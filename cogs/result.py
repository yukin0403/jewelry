...
class UserInputSelect1(discord.ui.Select):
    def __init__(self, bot: commands.Bot, options: list[discord.SelectOption] = ...) -> None:
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if room_role in result_list:
            await g.result_ch.send(f"重複した入力です。確認お願いします。\n"
                                   f"{g.admins_mention}\n"
                                   f"----------------------------------------\n"
...
class WinUmaView(discord.ui.View):
    def __init__(self, bot: commands.Bot, options: list[discord.SelectOption], result_info: list):
...
class result(commands.Cog):
    ...
    async def result(self, ctx):
        ...
        if not(win_member_id in id_list_str):
            await ctx.interaction.response.send_message(f"ウマ娘登録がされていません。", ephemeral=True)
            return
        else:
            ...
            await ctx.interaction.response.send_message(f"1位のウマ娘の名前を入力してください。", 
    ...
    async def on_result_error(self, ctx: commands.Context, error):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            ...
            return await ctx.interaction.response.send_message(f"クールダウン中です。{retry_after_int}秒後使用可能です。", ephemeral=True)
