...
class MyModal(discord.ui.Modal):
    def __init__(self, uma_list: list, user_list: list, *args, **kwargs) -> None:
        ...
        self.add_item(discord.ui.InputText(label="トレーナーID",
                                           min_length=9,
                                           max_length=12))
        self.add_item(discord.ui.InputText(label="意気込み", 
                                           required=False,
                                           placeholder="任意",
                                           style=discord.InputTextStyle.long))
        self.add_item(discord.ui.InputText(label="登録ウマ娘(確認用)", 
                                           required=False,
                                           style=discord.InputTextStyle.long,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        new_raw = [timestamp, club_name, user_name, role, un1, up1, un2, up2, un3, up3, trainer_id, user_id, enthusiasm]
        # サークル名リストを読み込んで入力したサークル名と一致するか調べる
        club_name_list = g.club_data['club_name'].tolist()
        if not(club_name in club_name_list):
            await interaction.response.send_message("一致するサークル名がありません。\n"
                                                    "登録したサークル名を確認するか、サークル登録を行ってからウマ娘登録を実行してください。", ephemeral=True)
            return
        else:
            # スプシに書き込み
            ...
            await interaction.response.send_message(f"サークル名:{club_name}\n"
                                                    ...
                                                    f"ウマ娘2:{un2} ({up2})\n"
                                                    f"ウマ娘3:{un3} ({up3})\n"
                                                    f"トレーナーID:{trainer_id}", ephemeral=True)
...
class uma_register(commands.Cog):
    ...
    async def register(self, ctx,
        ...
        uma_ids = [unicodedata.normalize('NFKC', uma_id) for uma_id in [ウマ娘id1, ウマ娘id2, ウマ娘id3]]
        uma_names = [g.uma_list_mapping.get(uma_id) for uma_id in uma_ids]
        # ウマ娘名が存在しない場合
        if any(uma_name is None for uma_name in uma_names):
            await ctx.interaction.response.send_message("存在しないウマ娘IDが指定されています。", ephemeral=True)
            return
        # ほしい要素をリストに格納してモーダルに送信
        else:
            uma_list = [uma_names[0], ウマ娘脚質1, uma_names[1], ウマ娘脚質2, uma_names[2], ウマ娘脚質3] 
